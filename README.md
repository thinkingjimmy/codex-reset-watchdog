...
Open the Project you just created, start a new chat inside that Project, then paste the prompt below. Codex will install this skill, run the basic checks, and prime baseline state, but it will not create an Automation.

```text
Quietly install and initialize codex-reset-watchdog inside the current Codex Project:
...
9. Do not create, update, or test an Automation; I will create it manually in the next step to avoid tool-layer retries leaving invalid Automations behind.
10. Final summary only: install directory, self-test, prime/dry-run status, state.path, source health, and the working directory I should use when creating the Automation. Do not paste raw JSON.
```
...
4. Working directory/cwds: the installed skill directory from Step 2, the one containing `SKILL.md`, `scripts/check_once.mjs`, and `.codex/config.toml`.
5. Prompt: copy the block below. It must stay the same as `references/automation-prompt.md`.
